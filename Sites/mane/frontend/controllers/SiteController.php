...
class SiteController extends Controller
{
    ...
    public function actionSendSms()
    {
        ...
        if (!empty($_POST['phone'])) {
            if (empty($_SESSION['exp_date_sms']) || time() > $_SESSION['exp_date_sms']) {
                $phone = preg_replace('/[^\d]+/', '', Html::encode($_POST['phone']));
                $code_send = hexdec(substr(md5($phone . "der_code"), 7, 5));
                ...
                curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array(
                    ...
                    "msg" => "Юридическая компания PRAVO M. Ваш код для авторизации: {$code_send}",
                    "json" => 1
                )));
                curl_exec($ch);
                curl_close($ch);
    ...
    public function actionIndex()
    {
        ...
        $services = Services::find()->orderBy(['date' => SORT_DESC])->limit(4)->asArray()->all();
        $reviews = Reviews::find()->orderBy('id desc')->limit(3)->asArray()->all();
        $completed = Completed::find()->orderBy('id desc')->asArray()->all();
        return $this->render('index', ['services' => $services, 'reviews' => $reviews, 'completed' => $completed]);
    }
    ...
    public function actionLike()
    {
        ...
        if (!empty($_POST['like']) && !empty($_POST['id'])) {
            $article = Articles::findOne(['id' => $_POST['id']]);
            if (!empty($article)) {
                $cookie = $_COOKIE['like'];
                $id = $article->id;
                $cookiediz = true;
                $diz = json_decode($_COOKIE['diz'], true);
                if (in_array($id, $diz)) {
                    $cookiediz = false;
                }
                if ($cookiediz) {
                    if (empty($cookie)) {
                        $cookieid = json_encode([$id], JSON_UNESCAPED_UNICODE);
                        setcookie('like', $cookieid, time() + 3600 * 24 * 365 * 10, '/');
                        ...
                        if (!in_array($id, $arr)) {
                            $arr[] = $id;
                            $cookie = json_encode($arr, JSON_UNESCAPED_UNICODE);
                            setcookie('like', $cookie, time() + 3600 * 24 * 365 * 10, '/');
                            $article->likes = $article->likes + 1;
                            $article->update();
                        } else $rsp = ['status' => 'error', 'message' => 'Лайк уже поставлен'];
                    }
                } else $rsp = ['status' => 'error', 'message' => 'Статья вам уже не понравилась'];
            } else $rsp = ['status' => 'error', 'message' => 'Нет данных'];
        } else $rsp = ['status' => 'error', 'message' => 'Нет данных'];
        return $rsp;
    }
    ...
    public function actionDislike()
    {
        ...
        if (!empty($_POST['like']) && !empty($_POST['id'])) {
            ...
            if (!empty($article)) {
                ...
                $id = $article->id;
                $cookiediz = true;
                $diz = json_decode($_COOKIE['like'], true);
                if (in_array($id, $diz)) {
                    $cookiediz = false;
                }
                if ($cookiediz) {
                    if (empty($cookie)) {
                        $cookieid = json_encode([$id], JSON_UNESCAPED_UNICODE);
                        setcookie('diz', $cookieid, time() + 3600 * 24 * 365 * 10, '/');
                        $article->dizlike = $article->dizlike + 1;
                        $article->update();
                    } else {
                        $arr = json_decode($cookie, true);
                        if (!in_array($id, $arr)) {
                            $arr[] = $id;
                            $cookie = json_encode($arr, JSON_UNESCAPED_UNICODE);
                            setcookie('diz', $cookie, time() + 3600 * 24 * 365 * 10, '/');
                            $article->dizlike = $article->dizlike + 1;
                            $article->update();
                        } else $rsp = ['status' => 'error', 'message' => 'Дизлайк уже поставлен'];
                    }
                } else $rsp = ['status' => 'error', 'message' => 'Статья вам уже понравилась'];
            } else $rsp = ['status' => 'error', 'message' => 'Нет данных'];
        } else $rsp = ['status' => 'error', 'message' => 'Нет данных'];
        return $rsp;
    }
